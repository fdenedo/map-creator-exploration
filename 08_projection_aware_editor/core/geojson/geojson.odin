...
GeoJsonParseErrorCategory :: enum {
    None,
    Invalid_JSON,
    Missing_Field,
    Invalid_Type,
...
GeoJsonParseError :: struct {
    category: GeoJsonParseErrorCategory,
    message:  string,
...
parse_geojson :: proc(data: []byte, allocator := context.allocator) -> (result: GeoJSON, err: GeoJsonParseError) {
    ...
    if json_err := json.unmarshal(data, &raw_value, allocator = context.temp_allocator); json_err != nil {
        return GeoJSON {}, GeoJsonParseError {
...
raw_to_feature_collection :: proc(raw: Raw_FeatureCollection) -> (result: FeatureCollection, err: GeoJsonParseError) {
    ...
    }, GeoJsonParseError{category = .None}
}

raw_to_feature :: proc(raw: Raw_Feature) -> (result: Feature, err: GeoJsonParseError) {
    feature := Feature {
    ...
    return feature, GeoJsonParseError { category = .None }
}

raw_to_geometry :: proc(raw: Raw_Geometry) -> (result: Geometry, err: GeoJsonParseError) {
    bbox := parse_bbox(raw.bbox)
    ...
    case "Point":
        ...
        if !coords_ok {
            return Geometry {}, GeoJsonParseError {
                category = .Missing_Field,
        ...
        return Point{ coordinates = pos, bbox = bbox }, GeoJsonParseError { category = .None }
    ...
    case "MultiPoint":
        ...
        return MultiPoint { coordinates = positions, bbox = bbox }, GeoJsonParseError { category = .None }
    ...
    case "LineString":
        ...
        return LineString { coordinates = positions, bbox = bbox }, GeoJsonParseError { category = .None }
    ...
    case "MultiLineString":
        ...
        return MultiLineString { coordinates = lines, bbox = bbox }, GeoJsonParseError { category = .None }
    ...
    case "Polygon":
        ...
        return Polygon { coordinates = rings, bbox = bbox }, GeoJsonParseError { category = .None }
    ...
    case "MultiPolygon":
        ...
        return MultiPolygon { coordinates = polygons, bbox = bbox }, GeoJsonParseError { category = .None }
    ...
    case "GeometryCollection":
        ...
        return GeometryCollection { geometries = geometries, bbox = bbox }, GeoJsonParseError { category = .None }
...
parse_line_array :: proc(val: json.Value) -> ([][]Position, GeoJsonParseError) {
    arr, arr_ok := val.(json.Array)
    if !arr_ok {
        return nil, GeoJsonParseError {
            category = .Invalid_Type,
    ...
    return lines, GeoJsonParseError { category = .None }
}

parse_ring_array :: proc(val: json.Value) -> ([]LinearRing, GeoJsonParseError) {
    arr, arr_ok := val.(json.Array)
    if !arr_ok {
        return nil, GeoJsonParseError {
            category = .Invalid_Type,
    ...
    return rings, GeoJsonParseError { category = .None }
}

parse_polygon_array :: proc(val: json.Value) -> ([][]LinearRing, GeoJsonParseError) {
    arr, arr_ok := val.(json.Array)
    if !arr_ok {
        return nil, GeoJsonParseError {
            category = .Invalid_Type,
    ...
    return polygons, GeoJsonParseError { category = .None }
}
