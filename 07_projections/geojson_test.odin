...
@(test)
test_polygon_with_unclosed_ring :: proc(t: ^testing.T) {
    allocator, arena, buffer := test_arena_allocator()
    defer cleanup_test_arena(arena, buffer)

    // A ring is closed if the first and last coordinates are the same
    json_data := `{
        "type": "Polygon",
        "coordinates": [
            [
...
@(test)
test_polygon_with_small_ring :: proc(t: ^testing.T) {
    allocator, arena, buffer := test_arena_allocator()
    defer cleanup_test_arena(arena, buffer)
